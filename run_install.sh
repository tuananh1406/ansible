...
if [ -z $1 ];
then
        echo -e "\e[1m\e[91m\e[100mThiếu tham số. Chạy '$0 python' hoặc '$0 gdnsd'\e[0m";
        exit 255;
fi
...
# Bật ssh
echo -e "\e[1m\e[91m\e[100mKiểm tra SSH\e[0m"
eval `ssh-agent -s` > /dev/null
ssh-add -q $PATH_SSH_KEY

echo -e "\e[1m\e[91m\e[100mXoá instance $INSTANCE_NAME (nếu có)\e[0m"
multipass delete $INSTANCE_NAME
multipass purge

echo -e "\e[1m\e[91m\e[100mKhởi tạo instance $INSTANCE_NAME mới\e[0m"
...
echo -e "\e[1m\e[91m\e[100mLấy ipv4 của instance mới\e[0m"
IP=$(multipass info ${INSTANCE_NAME} |grep IPv4|awk '{print $2}')
echo -e "\e[1m\e[91m\e[100mIPv4: $IP\e[0m"

echo -e "\e[1m\e[91m\e[100mThêm instance vào ansible host\e[0m"
...
then
        # Cài python 3.6
        echo -e "\e[1m\e[91m\e[100mKiểm tra python 3 trên instance\e[0m"
        multipass exec ${INSTANCE_NAME} -- python3  --version
        multipass exec ${INSTANCE_NAME} -- bash -c 'ls /usr/local/bin/python3.6'

        echo -e "\e[1m\e[91m\e[100mCài python3.6.12 bằng ansible\e[0m"
        ansible-playbook -i hosts worker.yml -l test-1 -t install_python

        echo -e "\e[1m\e[91m\e[100mKiểm tra python 3 trên instance sau khi cài\e[0m"
        multipass exec ${INSTANCE_NAME} -- bash -c 'ls /usr/local/bin/python3.6'
